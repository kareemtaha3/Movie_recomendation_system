...
"""
Training pipeline for the embedding neuro-fuzzy movie recommendation system.
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description='Train the embedding neuro-fuzzy recommendation model')
...
def train_model(config, movie_features, user_features, ratings_df):
    ...
    """
    logger = logging.getLogger('movie_recommender.pipeline')
    logger.info("Training embedding neuro-fuzzy model")
    
    # Initialize model with embedding configuration
    model_config = config.get('model', {})
    embedding_config = model_config.get('embedding', {})
    
    # Create a combined configuration for the embedding neuro-fuzzy model
    combined_config = {
        'embedding_dim': embedding_config.get('embedding_dim', 32),
        'hidden_layers': embedding_config.get('hidden_layers', [64, 32]),
        'dropout_rate': embedding_config.get('dropout_rate', 0.2),
        'learning_rate': embedding_config.get('learning_rate', 0.001),
        'activation': embedding_config.get('activation', 'relu'),
        'l2_regularization': embedding_config.get('l2_regularization', 0.01),
        'fuzzy_membership_funcs': model_config.get('fuzzy', {}).get('membership_funcs', 3),
        'fuzzy_rules': model_config.get('fuzzy', {}).get('rules', 'auto'),
        'neural_fuzzy_weight': model_config.get('fuzzy', {}).get('neural_fuzzy_weight', 0.7)
    }
    
    # Initialize model
    model = EmbeddingNeuroFuzzyRecommender(combined_config)
    ...
    model_path = os.path.join(model_dir, 'embedding_neuro_fuzzy_model')
    ...
    try:
        fuzzy_plot = model.plot_fuzzy_membership()
        fuzzy_plot_path = os.path.join(figures_dir, 'fuzzy_membership.png')
        fuzzy_plot.savefig(fuzzy_plot_path)
        plt.close(fuzzy_plot)
    except Exception as e:
        logger.warning(f"Could not plot fuzzy membership functions: {e}")
...
def evaluate_model(config, model, movie_features, user_features, ratings_df):
    ...
    config : dict
        Configuration parameters.
    model : EmbeddingNeuroFuzzyRecommender
        Trained model.
    movie_features : pandas.DataFrame
        DataFrame containing movie features.
    user_features : pandas.DataFrame
    ...
    sample_users = np.random.choice(user_features['userId'].unique(), 
                                min(5, len(user_features['userId'].unique())), 
                                replace=False)

    recommendations_dir = os.path.join(metrics_dir, 'sample_recommendations')
    ...
    for user_id in sample_users:
        try:
            ...
            logger.info(f"Saved recommendations for user {user_id} to {recommendations_path}")
        except Exception as e:
...
def generate_visualizations(config, metrics, predictions_df, movie_features, user_features, ratings_df):
    ...
    visualizer.plot_rating_distribution(
        ...
        save_as="embedding_rating_distribution.png"
    )
    ...
    visualizer.plot_prediction_vs_actual(
        ...
        title="Predicted vs Actual Ratings (Embedding Neuro-Fuzzy)",
        save_as="embedding_predicted_vs_actual.png"
    )
    ...
    visualizer.plot_error_distribution(
        predictions_df['actual_rating'],
        predictions_df['predicted_rating'],
        title="Error Distribution (Embedding Neuro-Fuzzy)",
        save_as="embedding_error_distribution.png"
    )
    
    # Plot recommendation metrics
    visualizer.plot_recommendation_metrics(
        metrics,
        title="Recommendation Metrics (Embedding Neuro-Fuzzy)",
        save_as="embedding_recommendation_metrics.png"
    )
    ...
    visualizer.plot_confusion_matrix(
        ...
        predictions_df['predicted_rating'],
        threshold=config.get('evaluation', {}).get('like_threshold', 3.5),
        title="Confusion Matrix (Embedding Neuro-Fuzzy)",
        save_as="embedding_confusion_matrix.png"
    )
    ...
    visualizer.plot_precision_recall_curve(
        ...
        predictions_df['predicted_rating'],
        threshold=config.get('evaluation', {}).get('like_threshold', 3.5),
        title="Precision-Recall Curve (Embedding Neuro-Fuzzy)",
        save_as="embedding_precision_recall_curve.png"
    )
    ...
    visualizer.plot_roc_curve(
        ...
        predictions_df['predicted_rating'],
        threshold=config.get('evaluation', {}).get('like_threshold', 3.5),
        title="ROC Curve (Embedding Neuro-Fuzzy)",
        save_as="embedding_roc_curve.png"
    )
...
def main():
    ...
    log_file = os.path.join(log_dir, f"embedding_train_pipeline_{time.strftime('%Y%m%d_%H%M%S')}.log")
    logger = setup_logging(log_file=log_file, log_level=getattr(logging, args.log_level))
    
    logger.info("Starting embedding neuro-fuzzy training pipeline")
    ...
    try:
        ...
        # Train model
        model, history = train_model(config, movie_features, user_features, ratings_df)
        ...
        logger.info("Embedding neuro-fuzzy training pipeline completed successfully")
        
    except Exception as e:
        logger.exception(f"Error in embedding neuro-fuzzy training pipeline: {e}")
        sys.exit(1)
